...
class WorkbenchRebuildTests(unittest.TestCase):
    ...
    def test_new_book_keeps_opening_fields_and_chinese_pov_label(self):
        with IsolatedEnv():
            ...
            book_id = repo.create_book(
                ...
                worldbuilding="词条有代价",
                book_outline="全书结构化大纲",
                imported_outline="旧草案",
                estimated_total_words=800000,
            ...
            self.assertEqual("全书结构化大纲", record["book_outline"])
            self.assertEqual(800000, record["estimated_total_words"])
            self.assertEqual("第一人称主角视角", controls["pov_label"])

    def test_outline_lock_prevents_normal_outline_overwrite(self):
        with IsolatedEnv():
            repo = self.repo()
            book_id = repo.create_book("锁定书", "测试", book_outline="原始书纲")
            repo.lock_book_outline(book_id)
            repo.update_book_setup(book_id, {"title": "锁定书", "premise": "测试", "book_outline": "被覆盖", "estimated_total_words": 900000})
            self.assertEqual("原始书纲", repo.get_book_record(book_id)["book_outline"])
            repo.unlock_book_outline(book_id)
            repo.update_book_setup(book_id, {"title": "锁定书", "premise": "测试", "book_outline": "新书纲", "estimated_total_words": 900000})
            self.assertEqual("新书纲", repo.get_book_record(book_id)["book_outline"])

    def test_book_author_editor_overrides_do_not_mutate_global_resources(self):
        with IsolatedEnv():
            repo = self.repo()
            book_id = repo.create_book("资源覆盖书", "测试资源")
            author_id = repo.save_profile("authors", {"name": "资源作者", "genre": "都市", "pov_preference": "first_person", "sentence_rhythm": "口语化", "dialogue_style": "直接", "payoff_preference": "反击", "forbidden_items": "说明书", "prompt_rules": "用我叙述"})
            editor_id = repo.save_profile("editors", {"name": "资源编辑", "platform": "新平台", "word_count_rule": "2000 字以上", "pov_rule": "跟随作品", "structure_rule": "起因经过结果", "payoff_rule": "有爽点", "pollution_rule": "无部门话术", "reject_threshold": 1})
    ...
    def test_batch_uses_unit_recommended_count_and_binds_volume_unit(self):
        with IsolatedEnv():
            repo = self.repo()
            book_id = repo.create_book("批次书", "测试批次")
            arc = repo.list_arcs(book_id)[0]
            repo.update_arc(book_id, int(arc["id"]), {**arc, "recommended_chapters": 4})
            batch_id = repo.create_chapter_batch(book_id)
            batch = repo.get_chapter_batch(batch_id)
            plans = repo.list_chapter_plan_rows(book_id, batch_id)
            self.assertEqual((1, 4), (batch["start_chapter"], batch["end_chapter"]))
            self.assertEqual(4, batch["recommended_count"])
            self.assertTrue(all(row["volume_id"] == batch["volume_id"] and row["arc_id"] == batch["arc_id"] for row in plans))
    ...
    def test_ref_pack_is_bounded_and_logs_retrieval(self):
        with IsolatedEnv():
            ...
            self.assertLessEqual(ref_pack.content.count("chapter_memory"), 10)
    ...
    def test_unapproved_atoms_are_not_used_in_ref_pack(self):
        with IsolatedEnv():
            ...
            repo.create_chapter_batch(book_id, 2)
